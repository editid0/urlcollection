...
export default function EditForm({ collection_id, url_id, data }) {
    const [title, setTitle] = useState(data.title || "");
    const [description, setDescription] = useState(data.description || "");
    const [url_, setUrl] = useState(data.url || "");
    ...
    function handleSubmit(event) {
        ...
        // collection_id, url_id, title, description, url
        const formData = {
            collection_id,
            url_id,
            title,
        ...
        fetch('/api/urls/edit', {
            ...
            .then(async response => {
                if (response.ok) {
                    ...
                    setDescription("");
                    setUrlValid(null);
                    window.location.href = `/collections/${collection_id}`;
                } else {
                    const text = await response.text();
                    alert(`Error: ${text}`);
                }
